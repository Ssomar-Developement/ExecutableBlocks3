...
import com.ssomar.scoretestrecode.features.types.SlotFeature;
...
public class CreationTypeFeature extends FeatureAbstract<Optional<CreationType>, CreationTypeFeature> implements FeatureRequireOnlyClicksInEditor {
    ...
    public CreationTypeFeature(FeatureParentInterface parent, String name, Optional<CreationType> defaultValue, String editorName, String[] editorDescription, Material editorMaterial, boolean requirePremium) {
    ...
    @Override
    public CreationTypeFeature clone() {
        CreationTypeFeature clone = new CreationTypeFeature(getParent(), this.getName(), getDefaultValue(), getEditorName(), getEditorDescription(), getEditorMaterial(), requirePremium());
        clone.value = value;
        return clone;
    }
    ...
    public void updateCreationType(CreationType slot, GUI gui) {
        ...
        || (slot.equals(CreationType.IMPORT_FROM_ITEMS_ADDER) && !ExecutableBlocks.hasItemsAdder)
        || (slot.equals(CreationType.IMPORT_FROM_ORAXEN) && !ExecutableBlocks.hasOraxen)){
